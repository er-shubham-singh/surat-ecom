...
import { logout as logoutAction } from "../redux/Auth/action"; // ensure path is correct
...
/* SVG icons omitted for brevity - reuse yours exactly as before */
            ...
            <div>
              <Link to="/" className="flex items-center space-x-3 group">
                <div className="relative">
                  ...
                  <img src="logo/logo.jpeg" alt="Venus Garments Logo" className="h-12 rounded-full object-contain transition-transform duration-300 group-hover:scale-105" />
                </div>
              </Link>
            </div>
...
<nav className="hidden md:flex flex-1 justify-center px-8 relative">
  <ul className="flex items-center gap-1 font-semibold uppercase tracking-wider text-sm lg:text-base">
    {navigation.map((nav) => {
      ...
      // NOTE: removed `relative` from li so dropdown positions relative to nav (which is now relative)
      return (
        <li key={nav.id} onMouseEnter={() => handleMouseEnter(nav.id)} onMouseLeave={handleMouseLeave}>
          {isMega ? (
            <button
              onClick={(e) => handleClickToggle(e, nav.id)}
              className={`flex items-center gap-2 px-5 py-3 rounded-lg hover:bg-[#CBE600] transition-all duration-300 font-bold ${isOpen ? "bg-[#CBE600] shadow-lg" : ""}`}
          ...
          {isMega && (
                ...
                <div className="grid grid-cols-12 gap-8">
                      ...
                      <div key={cat.id} className="space-y-1">
                        ...
                        <ul className="space-y-1">
                          {cat.subHeadings?.map((item) => (
                            <li key={item.id}>
                              <Link to={item.path || item.href || "#"} className="block text-[#666] hover:text-[#CBE600] transition-colors text-sm leading-relaxed">
                                {item.name}
                              </Link>
                            </li>
                          ))}
                  ...
                  <div className="col-span-3 flex items-center justify-center">
                    {Array.isArray(nav.featured) && nav.featured.length > 0 ? (
                      <Link to={nav.featured[0].href || nav.path || "#"} className="block rounded-lg overflow-hidden shadow-lg hover:shadow-xl transition-all duration-300 w-full">
                        <img src={nav.featured[0].imageSrc} alt={nav.featured[0].name} className="w-full h-full object-cover hover:scale-105 transition-transform duration-500" />
                      </Link>
                    ) : Array.isArray(nav.images) && nav.images.length > 0 ? (
                      <Link to={nav.path || "#"} className="block rounded-lg overflow-hidden shadow-lg hover:shadow-xl transition-all duration-300 w-full">
                        <img src={nav.images[0]} alt={`${nav.title} promotion`} className="w-full h-full object-cover hover:scale-105 transition-transform duration-500" />
                      </Link>
                    ) : null}
                  </div>
                </div>
            ...
            <div className="flex items-center gap-2">
              ...
              <div className="relative" ref={profileRef}>
                <button
                  aria-label="Account"
                  className={`${iconButtonClass} ${isAuthenticated ? "px-1" : ""}`}
                  onClick={() => {
                    if (!isAuthenticated) {
                    ...
                    // toggle small dropdown when authenticated
                    setProfileOpen((p) => !p);
                  }}
                >
                ...
                {isAuthenticated && (
                  <div className={`absolute right-0 mt-2 w-40 rounded-md shadow-lg bg-white border z-50 transition-transform origin-top-right ${profileOpen ? "scale-100 opacity-100" : "scale-95 opacity-0 pointer-events-none"}`}>
                    <div className="p-2">
                      <div className="px-3 py-2 text-sm text-gray-700 font-medium">Hello, {displayName}</div>
                      <div className="border-t my-1" />
                      <button
                        onClick={() => {
                          setProfileOpen(false);
                          navigate("/profile"); // optional profile route
                        }}
                        className="w-full text-left px-3 py-2 rounded hover:bg-gray-100 text-sm"
                      >
                        Profile
                      </button>
                      <button onClick={handleLogout} className="w-full text-left px-3 py-2 rounded hover:bg-gray-100 text-sm text-red-600 font-medium">
                        Logout
                      </button>
                    </div>
                  </div>
...
<div className={`fixed top-0 left-0 h-full w-80 max-w-[85vw] z-50 md:hidden transition-transform duration-300 ${mobileMenuOpen ? "translate-x-0" : "-translate-x-full"}`}>
  ...
  <div className="h-full overflow-y-auto bg-white">
    ...
    {/* Mobile Navigation — colors & hover match desktop, removed borders */}
          ...
          return (
            <li key={nav.id}>
              {isMega ? (
                    ...
                    <div className="pl-2 pr-2 space-y-3">
                      {nav.categories?.map((cat) => (
                        <div key={cat.id} className="">
                          <h4 className="text-xs font-bold uppercase tracking-wider text-[#CBE600] mb-2">{cat.title}</h4>
                          <ul className="space-y-1">
                            {cat.subHeadings?.map((item) => (
                              <li key={item.id}>
                                <Link to={item.path || item.href || "#"} onClick={() => { setMobileMenuOpen(false); setOpenMenu(null); }} className="block px-3 py-2 rounded-lg hover:bg-[#DFF200]/40 transition-colors text-sm font-medium">
                                  {item.name}
                                </Link>
                              </li>
                            ))}
                ...
                <Link to={nav.path} onClick={() => setMobileMenuOpen(false)} className="block px-4 py-3 rounded-lg hover:bg-[#DFF200]/40 transition-colors font-semibold uppercase text-sm">
                  {nav.title}
                </Link>
              )}
            </li>
    ...
    {/* Mobile Contact Info (simple, matching tone) */}
